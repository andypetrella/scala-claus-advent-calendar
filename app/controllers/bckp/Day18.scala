...
case class Day18[A](parser: BodyParser[A]) extends DayTmpl[A, String] {
  ...
  def sync: String = {
    s"""
      What about the 1 billion dollar mistake ${???}
    """
    ...
    object Person {
      ...
      def randomCountry:Country = if (nextBoolean()) Belgium("FR") else if (nextBoolean()) Belgium("NL") else if (nextBoolean()) England else if (nextBoolean()) France else null
